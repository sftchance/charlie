...
(
	SELECT  COUNT(DISTINCT delegator) AS total_unique_delegators
	FROM
	(
		SELECT  event_inputs:"delegator" AS delegator
		FROM polygon_contracts
		UNION ALL
		SELECT  event_inputs:"delegator" AS delegator
		FROM arbitrum_contracts
...
SELECT  combined_delegators.*
       ,total_delegators.total_unique_delegators
FROM combined_delegators
CROSS JOIN total_delegators
ORDER BY 1 DESC